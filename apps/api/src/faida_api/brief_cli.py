...
Two things, both of which the product needs before Meta has approved
...
vertical slice). The second is the rehearsal: one real template send to one
phone at any hour, which is how the sitting proves the template against the
real number without waiting for a morning (§8 step 5).

It composes nothing of its own. `worker.read_brief` is the one function that
makes the two dashboard reads and calls the filler, and this calls it - so
what is printed here is what the job sends there, by construction and not by
agreement. A rehearsal is recorded like any other send, with `rehearsal: true`
on the row, which keeps it outside the day's key: a rehearsal at four in the
afternoon must not silence the next morning's brief (`db.outbound_brief_exists`).
...
async def run(
    ...
    *,
    ...
    """Print one morning's brief, and send it if asked. Returns the message id
    of a rehearsal send, or None.

    Takes its database and its Graph client rather than building them, so the
    test drives the same function the terminal does against the test database
    and a mocked Meta."""
    ...
    if send_to is None:
        return None
    if wa is None:
        raise ValueError("a rehearsal send needs a WhatsApp client")
    message_id = await wa.send_template(
        send_to, brief.TEMPLATE_NAME, brief.TEMPLATE_LANGUAGE, morning.parameters
    )
...
async def _run_from_settings(args: argparse.Namespace) -> None:
    ...
    try:
        await run(
            db, wa, tenant_id=args.tenant, today=today, send_to=args.to if args.send else None
        )
    finally:
        if wa is not None:
            await wa.close()
