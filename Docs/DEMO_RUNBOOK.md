...
**The M6 demo gate is a different and later claim**: the full script, both acts, twice in a row with zero intervention, on **F7's real menu** rather than the staged one. **Founder's amendment, 2026-09-01: the meme beat (§B steps 13-15) is waived from the gate rehearsals** - the decline path was separately signed off on Flash on 2026-08-30 (§E0, word-perfect) and does not need re-proving; it stays in the on-stage script. Row 3 ran under that amendment: KAS-5 through the loop on the real menu, act two on the reloaded screens, zero intervention. Fill in row 4 when run 2 happens.
