...
Status: **built 2026-09-08 on the integration branch `m10-build` by six lanes (rows 100 to 103 and 106; 1,013 tests green on the combined branch, 117 new), the template's approval (row 104) and the live sitting (row 105) still open; the lanes' decisions are in `plan.md` §7.3's rows and one Decision Log row.** Approved to build 2026-09-08: every §5 proposal decided by the founder - the ten open ones as recommended in one word ("ok take all 10 as recommended", after each was put in plain English at https://claude.ai/code/artifact/8b7cca7a-49f8-458c-b3ba-4d4775f2d912), P15 the picture card, P16 the recipe cost, P17 items by money kept; C15 and the C2 amendment are pinned in `plan.md` §7.2 and the rows in §7.3 are the build order.** Redrawn earlier the same morning to the founder's own KPI direction (§1). Before that, decomposed and reviewed 2026-09-07 with one outside voice (Codex at medium reasoning against the file path, read-only: thirteen findings, thirteen folded, four of them blockers that changed the queue's story, the migration's number and two founder decisions), awaiting the founder's decisions.**
...
Every §5 proposal is now decided; the report at the end records each decision, and the rows in §4 are the rows in `plan.md` §7.3, approved to build.
...
Recorded in `plan.md` on 2026-09-08: C15 and the C2 amendment pinned in §7.2, the rows in §7.3 approved, the M10 header in §8, the Decision Log rows, and the deferred items in `TODOS.md`.
...
## 3. Contracts to pin before fan-out (C15 new; C2 amended; C7 one migration; C8 extended; C6 extended by two fields; C10 and C13 untouched) - pinned 2026-09-08
...
**Template A2 - the account-statement wording for production** (P13 as amended 2026-09-11; the same five variables and the same image header; name `faida_account_statement`, category Utility, language English; submitted under the production number, not the test one):

```
*Your Faida account statement*
{{1}}
...
This statement is for your Faida account: the card above carries every branch, the items and the price changes. Every figure opens to its source on the dashboard.
```

The words that changed are the ones Meta's reviewer reads as a newsletter - "morning brief" - replaced by what the message is to the person receiving it: a statement of their own account, which is the utility category's definition. The samples are the same five.
...
**Every proposal below is decided by the founder.** P15, P16 and P17 on the morning of 2026-09-08 as §1 records; the ten others - P1, P2, P4, P5, P6, P10, P11, P12, P13, P14 - the same morning, each put in plain English with its choices at https://claude.ai/code/artifact/8b7cca7a-49f8-458c-b3ba-4d4775f2d912 and taken as recommended in one word; P7 and P8 were the founder's answers at the start of the session; P3 and P9 are superseded by the founder's direction. The rows keep the reasoning; each recommendation is now the decision.
...
1. Now, under the test account: create `faida_daily_brief` in WhatsApp Manager from §3.1 (name, Utility, English, header Image with `Docs/brief/faida_daily_brief_sample.png` as the sample, the body of shape A, its five samples, the button); submit; note the date.
   **Done 2026-09-10:** submitted as Utility, English (`en`), with the sample card; "In review".
   **Approved 2026-09-11 as Marketing.** The founder accepted it for the pilot on the test number (free either way) and chose the account-statement wording (Template A2) for production under the real number (P13 amended; row 107). The app's WhatsApp overview showed business verification (Meta's step 3 below) already complete, so step 3 of this chain is done and step 4 is what remains for production.
...
1. **Wave 1 merges** (WP-100, WP-101): no schema, no behaviour change, the route byte-identical; Railway deploys; the founder runs `python -m faida_api.brief_cli --tenant <demo chain>` against the live database and reads the demo chain's brief.
...
| Eng Review | `/plan-eng-review` | Architecture & tests (required) | 0 | **APPROVED TO BUILD 2026-09-08, every §5 proposal decided by the founder** | the session ran unattended beside WP-96's sitting; its substance is §2 to §9 with the outside voice; a review with the founder present, as M9 and M12 had, is the next step and the §5 decisions are its agenda |
...
**VERDICT (2026-09-07):** **RECOMMENDED TO THE FOUNDER, NOT APPROVED.** Every outside-voice finding is folded or turned into a decision; the contracts in §3 are proposed, not pinned; the rows in §4 are not approved to build; no code exists. *(Superseded 2026-09-08: see the amended verdict at the end of the file.)*
...
**DECISIONS (the founder, 2026-09-08, "ok take all 10 as recommended"):** P1 the worker's tick; P2 the recipients table with the pilot's row by paste; P4 every morning once anything is loaded, the age in the header; P5 the login, no magic link; P6 manager briefs deferred; P10 one fixed reply a day to a recipient's message; P11 due at 07:00, skipped after noon; P12 the stale-running reclaim in WP-103; P13 production waits for a utility approval; P14 the pilot owner's row only with their request in hand. With P15, P16 and P17 above, every §5 proposal is decided and M10 is approved to build; C15 and the C2 amendment are pinned in `plan.md` §7.2.

**VERDICT, amended 2026-09-08:** **M10 APPROVED TO BUILD.** The one thing between here and Wave 1 is the manager's merge of this branch; Meta's review of the template runs beside it from the founder's submission.
