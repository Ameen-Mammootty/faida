...
Status: **eng-reviewed 2026-09-03** (`/plan-eng-review` with two outside voices, Codex and a Claude subagent; the report is at the end of this file), before any feature code; **awaiting the founder's decisions on §5 (P1-P7)** before the rows move into `plan.md` §7.3 as approved to build.
...
- **C11 - The sales row and the ratio.**
  1. *The canonical row is item-wise:* one row per (branch, business date, till item name, quantity, amount), from the till's item-wise report.
     ...
     A summary-only export (branch, business date, amount) is accepted by the same loader with no item column mapped and stored as a `summary` day with no lines; a summary day carries net sales and no coverage.
     A file is one shape throughout; two rows for one branch-day in a summary file stop the day with a sentence.
  ...
  8. *Coverage by sales value* over a period is the positive net value of lines whose till item maps to a menu item whose plate is not `incomplete`, over the positive net value of item-granularity lines that are not marked "not a menu item" - refund lines count in net sales and not in coverage, and a delivery charge is takings but not menu sales, so the figure is bounded 0-100% and can reach it (Codex 11, outside voice 8); summary days, refunds and non-menu value are reported beside the figure, never inside it.
...
- **C9 amended - completeness and freshness for period figures.**
  ...
  **A chain total row reconciles the table**: net sales, purchases and the ratio over every branch row plus the "No branch" group, and a test pins that the total equals the sum of the rows, so purchases cannot vanish between the papers and the screen (outside voice 4).
  The coverage figure carries the same rule: it is estimated when any counted plate is, and names the summary-only days beside it.
...
GET /api/sales/coverage?from&to
  {"period": {…}, "sales_value": "52000.00", "costed_value": "40560.00", "costed_pct": "78.0", "estimated_points": "12.0",
   ...
   "beside": {"refunds": "-210.00", "not_menu_items": "640.00", "summary_only_days": 0},
...
| Eng Review | `/plan-eng-review` | Architecture & tests (required) | 1 | CLEAR (PLAN), pending founder | 15 review findings, 27 outside-voice findings, 0 critical gaps open |
| Design Review | `/plan-design-review` | UI/UX gaps | 1 | PROPOSED (FULL), pending founder | score 4/10 to 9/10; 8 decisions taken as recommended; variant B proposed, wireframes written, no `approved.json` yet |
...
**VERDICT:** ENG CLEARED on the engineering questions and DESIGN PROPOSED - ready to move into `plan.md` §7.3 once the founder answers §5 and picks a wireframe variant. The CEO review is not required (the milestone is the plan's own checklist item with the founder's quote).

**UNRESOLVED DECISIONS:**
- P1 the CSV's canonical shape (item-wise canonical, summary accepted - recommended)
- P2 the purchase figure (total less printed VAT - recommended)
- P3 sales for all three branches in the demo week (recommended)
- P4 the seeded week as M8's gate, "real sales" moving to M11 (recommended)
- P5 Z-report photos deferred to TODOS with the trigger (recommended)
- P6 founder-track F9, one real till export before row 83 merges (recommended, non-blocking)
- P7 item-wise now with the ratio sequenced first, versus summary-only M8 (item-wise, sequenced - recommended)
- P8 the `/sales` wireframe variant (B "Answer first" - recommended; A and C on the same board)
