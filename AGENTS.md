...
2. `worker.py` is an asyncio loop in the same process polling the `jobs` table (`FOR UPDATE SKIP LOCKED` claim, 3 attempts, 30 s backoff). It downloads media promptly (Meta URLs expire), sha256-hashes it, stores the immutable original in Supabase Storage at `{tenant_id}/documents/{document_id}/original` (never overwritten, `x-upsert: false`; `storage.put_immutable` is the one write and the key builders beside it the one spelling of every path, and a key already taken answers `already_there`, which every caller treats as success because a retry meets its own first attempt), records the `documents` row, and sends the canned reply. Job handlers must be idempotent under retries. The worker fails closed (M7 WP-72, C2 as amended): `process_wa_message` is the one resolver from sender phone to branch and tenant, every job it enqueues carries `tenant_id` and `branch_id`, `extract_document` reads its document scoped by the payload's tenant and refuses a mismatch or a missing tenant by raising (the job fails, never guesses), and there is one extract job per document ever (`db.enqueue_once` against the 0018 index). A phone no branch is registered to gets its inbound `wa_messages` row stamped `ignored_unknown_sender` before anything else, one polite reply a day at most (best-effort), and nothing created.
...
Used is the recipes on file times the portions the till printed (refunds counted as made and named, P9), each component drawn by `plates.line_draw` - the typed quantity in base units divided by its optional usable share (`recipe_components.usable_share`, migration 0021, D13), the one answer the plate, contribution and a price move's impact ask too, so cost, quantity and a rise always describe the purchased amount (2026-09-24) - and by the yield, rounded once per dish to 0.001 base units; bought is every confirmed stock line in the branch's clipped window (`db.list_period_material_purchases`, line grain, price and currency never filters) times the frozen `cost_basis.pack_base_quantity` where the line was costed and `costing.resolve_pack` only where it was not (D2), rounded once per line, so every sum above is exact.
...
Phase one's consumer is `python -m faida_api.usage_report --tenant <id> --from --to [--branch]`, read-only, in three layers - `usage_inputs` (the period read, `period_read.read_period`, the same value the dashboard starts from, plus the one new read), `usage_blocks` (pure, §3.1's block) and `render` - with `usage_payload` ready to become the dashboard's `usage` block in phase two, which waits on the pilot's owner acting on the printout (D22) and a design review drawn from it (D23).

The quality word - reliable with limitations / estimated / incomplete / unavailable, never verified - is owned once by `quality.py` (2026-09-14): `Quality` is the one vocabulary a cost, a plate, a period row, a contribution, a usage row, a signal, the brief and the dashboard's freshness all carry, `worst` is C9's precedence, `total_of` is the 2026-09-04 rule for a total over rows, and `word` is the English; a reader never keeps its own rank table or its own string for the word, and its test asserts only that the worst input's word came through.

The period read - one tenant, one period, read once - is `period_read.read_period` (2026-09-14, CONTEXT.md "Period read"): the period the rule resolved, the branches, every branch's clipped window (`ratio.period_row` is called there and nowhere else), the unassigned papers and the chain total, the menu costed as of the period's last day (`menu.costed_menu`, a named `CostedMenu` since the same day) and the period's item sales, as one `PeriodRead`; the dashboard, `/sales/branches` and the usage printout start from it and derive their own figures after, so two screens over one period cannot disagree on a window or a plate, and the reads it makes are `period_read.READS`, the enumerated list every consumer's count test derives its maximum from.

A material's price in force - the newest delivery we could cost, estimated and naming the newer delivery when one could not be costed - is worked out once, in `price_in_force.py` (2026-09-26, CONTEXT.md "Price in force"): `read` makes the two reads, `from_rows` is the pure rule, `PriceInForce` carries the figure, the unit words ("per kg" everywhere, never "/kg") and `why_estimated`, and `payload` is its one wire shape; `menu.pricing`, the plates, contribution, usage and `/api/ingredients` all take it, and every price and invoice-line cost goes out through the one figure shape (`figure_of`, `figure_payload`); the Menu, Materials, invoice and dashboard screens print its `why_estimated` and `unit_words` as they arrive and compose neither, held by `priceWords.test.ts` over every mock.
...
The step after the read - validate, snap against the supplier's catalog, fold the snap onto each check without recomputing its status, the price alerts, the derived confidence and the line rows - is one pure module, `extraction/filing.py` (`file_invoice`; the word is CONTEXT.md's *Filing*, 2026-09-14), and the photo path, the correction door, the typed invoice and the eval's live runner all call it: until then each carried its own copy of the sequence and the copies had drifted, so the rule that a door cannot forget a step is now the module's shape, pinned by `tests/test_filing.py` on hand-built inputs. Normalization and the repair round run before it, the supplier lookup, the duplicate check, status and the write stay with each door.
