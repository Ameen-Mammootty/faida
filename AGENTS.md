...
The screen is `/dashboard`, where a fresh sign-in lands (`gate.ts`'s `DEFAULT_AFTER_LOGIN`, P10): the answer sentences, four tiles that are the row in view with the chain's figure named beside it, the branch league ranked by kept share lowest first with the bar drawn beside its own number, and every sentence that qualifies a figure behind an info icon (`InfoTip.tsx`, the founder's redesign of 2026-09-07), so the answer, the four figures and the whole league sit on one laptop screen; the branch view is that same screen filtered by `?branch=`, never a second route (P7).
