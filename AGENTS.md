...
Its only source until WP-70 is the legacy shared token resolving to the seeded tenant as actor `console`; the worker path still reads by id and is WP-72's lane. RLS stays deny-all: tenancy is enforced in the application layer by decision (2026-09-03 Decision Log), and policy-based RLS is owed at the first second door to the database (`TODOS.md`).
