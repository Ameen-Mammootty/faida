...
**M6 (recipes and menu costing)** - decomposed 2026-08-29, awaiting founder approval; no feature
code written.
The **DEMO GATE** closes this milestone: the full §6 script, twice in a row, zero intervention,
on the demo phones and a real menu (F7 supplies it).
...
| WP | What | Size | Depends | Acceptance |
|---|---|---|---|---|
| 60 | **The menu and its recipes exist, and every version of them survives.** `menu_items` (tenant, name, selling price as the owner states it) and recipes as **append-only version rows**: `recipes` (menu item, version, batch yield qty + unit) with `recipe_components` (ingredient, qty, unit) hanging off the version, batch yields the norm ("one pot → 40 cups", PRD §16). Editing writes a whole new version and never touches an old one, so "versioned" is a property of the schema rather than a subsystem: the current recipe is the newest version and the history is the table itself. Every write - menu item, price change, recipe version - is one transaction with one `audit_events` row naming its actor (C8; actors free text until M7). Selling-price history lives in those audit rows, not a price table, until a screen reads one (§2 rule 8; WP-55's precedent). A component on another tenant's ingredient is refused by **Postgres** (composite FK, the 0012 shape), and a component whose unit cannot convert to its ingredient's base unit is refused at write time with a plain sentence, never coerced - kitchen measures ("cup", "tbsp") are deliberately absent from `units.py` because a karak "cup" is a serving vessel, not a measure, and the consultant converts during loading (PRD §16's done-for-you defaults). Migration 0015. **Deliberately not built:** variants and modifiers (PRD §15 - the F7 input is one flat menu), per-branch menu items (§2 rule 8 - waits for a chain that shows different branch menus; WP-54's per-branch reasoning one layer up), delete/archive flags (nothing reads one) | M | M5 shipped | creating then editing a recipe leaves two versions with the old one byte-identical and the newest one current; a "2 cups milk" component is refused with a sentence naming the unit; a component on another tenant's ingredient raises in Postgres, not in Python; every write names its actor in one audit row |
| 61 | **Plate cost and margin - deterministic, labelled, and it names its inputs.** A pure module: each component's qty converts to its ingredient's base units (`units.py`), multiplies by that material's current price per base unit **read from WP-54's derivation as-is** (the same query, never a re-implementation), sums over components - packaging is just a component with a piece dimension - and divides once by the batch yield. `Decimal` throughout, quantized once at the end (WP-53's rounding rule). **Margin is computed against the price net of VAT and the screen says so in words:** GCC menu prices are displayed VAT-inclusive, so a 10.00 karak margins against 9.52 ex-VAT (C4's rate table) - margining against the gross would overstate every number in the demo by the VAT rate. Never labelled "food cost %" (§3). **C9 one layer up, mirroring WP-53:** the plate's quality is the worst label among its inputs - a component with no mapped material, no costed line yet, or any WP-55 blocked reason makes the item ***incomplete***, each missing thing named in plain words (reusing `BLOCKED_REASONS`); any *estimated* input makes the plate *estimated*; otherwise *reliable with limitations*. **No plate ever reads *verified***, pinned by a test, for WP-53's reason. An incomplete item has **no cost number at all** - it must never read as a cheap item - only its list of what is missing. Every answer carries the recipe version and, per component, the invoice line its price came from, which is what makes "why did this change?" answerable (PRD §8, §23) with no stored snapshot. **Deliberately not built:** stored plate costs (the block rule above), the PRD §23 calculation-run subsystem (it versions *sales-period* results; it arrives with sales in M8 - scope timing, this plan wins), yield-loss conversions (10 kg raw → 8.5 kg cooked - a recipe fact no menu in hand has shown yet; the batch yield already carries the pot-to-cups case) | M | 60 | hand arithmetic on a seeded karak matches to the fils; a 10.00 VAT-inclusive price margins against 9.52; one unmapped ingredient → *incomplete* with no cost shown; one *estimated* flour cost → an *estimated* plate; no plate reads *verified* (pinned); confirming a newer milk invoice moves the plate cost with **zero writes to any menu table**; the answer names its recipe version and each component's invoice line id |
| 62 | **The menu screen - the demo's closing image.** Every item with cost, margin in AED and margin % at its own menu price, **ranked by margin**; coverage by item count ("14 of 18 items costed" - by sales value waits for M8, per the checklist); and the drill: item → components → each with its qty, its material's price per kilo, its quality label and the invoice photo behind it, reusing the materials screen's affordances rather than inventing new ones. Incomplete items sit in their **own section** with what each is missing and one click to the fix (the mapping screen or the blocked-cost queue) - never in the ranking, where a missing cost reads as a fat margin. §3 display rules hold hardest here: rounded AED headlines, exact figures only in the drill, colour never carrying meaning alone | M/L | 61 | the ranking matches WP-61's hand-checks; an incomplete item appears only in its own section; any margin drills to an invoice photo in three clicks or fewer; nothing on the screen says "food cost" |
| 63 | **The money moment: a price move lands on the plates.** When a material's price moves, which menu items just lost margin and by how much - M2's price alert finally carried through to the plate: the alert names cartons, this names cups. Derived from the same WP-54 ordering read one row deeper (the newest costed purchase against the one before it), times each current recipe's qty per plate: what moved, since which invoice, which items, and the margin before and after in AED per plate. Because everything above the invoice line is derived, **confirming the rehearsal invoice at a new milk price re-ranks the karak line on the next screen read - no cache, no recompute job, nothing to invalidate** - which is the demo's act-two turn. **Deliberately not built:** margin alerts pushed to WhatsApp (M10's brief owns notifications), margin history over time (needs sales periods, M8/M9) | S/M | 61, 62 | confirm a seeded newer invoice with milk up → each affected item shows its exact AED drop and the unaffected items are byte-identical; unmap that material → the display corrects itself with no residue (WP-52's undo, visible at plate level) |
| 64 | **The batch loader: a real menu in a morning.** The internal consultant tool (PRD §16), in the web app behind the same shared secret: CSV in → a review grid showing every parsed row with its errors on it (an unknown ingredient offers to create it - the WP-52 approve-creates precedent; an unconvertible unit or a bad number blocks the row with a sentence) → commit writes through WP-60's one door, **one recipe = one transaction** (a recipe with a bad component does not half-load; the other recipes proceed), the consultant the recorded actor on every version. Re-uploading an edited CSV versions only the recipes that changed - an unchanged recipe produces no new version, so a re-import is not a history flood. The grid shows the running coverage count, which is the number a consultant works down. **Deliberately not built:** recipe templates across branches with per-branch overrides - **a proposed cut from this milestone's checklist, founder's call:** F7's input is one menu for one chain, per-branch menus wait for a chain that shows them (§2 rule 8), and template mechanics (which fields override, which follow) are exactly the machinery §2 bans ahead of a customer; menu-photo extraction tooling (PRD §15 keeps it consultant-facing; a consultant's spreadsheet is the path) | M/L | 60 | a 20-item CSV with two seeded errors loads 18 recipes and names both errors on their rows; fixing and re-uploading loads the two without duplicating the 18 or re-versioning the unchanged; the audit log names the consultant on every version; the seeded demo menu loads start to finish in one sitting |
...
Three decisions this decomposition puts to the founder, named plainly: the per-branch template cut
(WP-64), margin computed net of VAT (WP-61 - the alternative overstates every margin by the VAT
rate), and pulling EDGE-01 into the milestone (WP-65 - catalog hygiene, not menu code, but M6 is
where its damage lands).
...
Decomposed 2026-08-29 into WP-60 to WP-66 (§7.3), awaiting founder approval; three scope proposals
(the per-branch template cut, margin net of VAT, the EDGE-01 pull-in) are the founder's call.
