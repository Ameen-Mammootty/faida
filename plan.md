...
- 2026-08-29 - **The Gemini bake-off lane is built, fully tested, and proven against the live API as far as a free key allows; the measured comparison is waiting on one thing only - billing enabled on the founder's Gemini key, because the free tier serves Gemini 3 Pro at a limit of zero.**
  ...
  The founder's key was then confirmed against the live model list - `gemini-3.1-pro-preview` is the one plain Gemini 3 Pro id it serves, the provider's default - but the ten-case run stopped at quota: the key is free tier, and the free tier's limit for `gemini-3.1-pro` is zero requests, so Gemini 3 Pro needs billing enabled in Google AI Studio before a single invoice can be read (the same founder step F5 was for Anthropic).
  ...
  Next: with billing enabled on the key, `python -m eval.run --live --provider gemini --corpus eval/fixtures/generated` (twice if the first has per-case rate-limit failures), and report the side-by-side against the 2026-08-28 Opus baseline - accuracy, reconciliation, repair rounds, cost and latency - as the founder's decision input, not this lane's call.
