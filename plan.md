...
| 66 | **Demo assembly, and the gate itself.** `supabase/demo_seed.sql` gains a small menu with recipes over the already-seeded suppliers' materials, so act two rehearses before F7's menu lands - **the seed rehearses, the real menu gates**: the done-when ("one real menu loads in under a day") and the demo gate both close on F7's menu, not the seed. `Docs/DEMO_RUNBOOK.md` gains act two (materials screen → menu margins → the money moment → "push this, fix that") and its reset steps. Then the gate: the full §6 script, twice in a row, zero intervention, demo phones, real menu, on the shipped engine - which is why this row depended on M4's Flash re-run closing first - **it did, 2026-08-30, so what stands between here and the gate is M4's own two rehearsals and F7's menu loaded into the demo project** (M4's own box, not re-owned here) | S/M | 62-64, F7, M4's rehearsals | two consecutive clean full-script runs recorded in the runbook with per-stage timings, on Flash, on the real menu |
