...
- 2026-08-29 - **The Gemini bake-off lane is built and fully tested; the measured comparison itself is waiting on one thing only, the founder's Gemini API key.**
  ...
  Two findings from the wiring, ahead of any live call: our extraction schema crosses Gemini's structured-output wire natively with every money field as a string (the no-float-money rule holds), except the repair patch, which needs the same list-shaped workaround the Anthropic wire needed; and Gemini reports a truncated answer explicitly (`MAX_TOKENS`), so the provider raises on anything but a clean finish - WP-19's rule, a short read fails loudly, never a shorter invoice.
  ...
  Next: with `GEMINI_API_KEY` in the environment, list models to confirm the id, then `python -m eval.run --live --provider gemini --corpus eval/fixtures/generated` twice if the first has per-case rate-limit failures, and report the side-by-side against the 2026-08-28 Opus baseline - accuracy, reconciliation, repair rounds, cost and latency - as the founder's decision input, not this lane's call.
