...
--
-- ACT THREE (WP-85, M8): the sales week is NOT staged here. It is a till
-- export loaded through /sales/load, the door a pilot would use, from
-- Docs/demo-invoices/koukh-al-shay/sales-week-practice.csv - regenerated by
-- build_sales_week.py --practice, which reads the newest staged purchase day
-- below so the week lands on the same days as the papers. This reset deletes
-- the five sales tables for the demo chain (a complete practice reset), so
-- after every run of this file the week goes back in through the loader.
-- The demo's sales are invented; its purchases are not.
--
...
-- The sales week (M8, act three): lines go with their days, the till items
-- after the lines that point at them and before the menu items their
-- mappings point at, then the layouts and the aliases. A complete practice
-- reset - the week is re-uploaded through /sales/load afterwards.
...
delete from till_items
 where tenant_id = 'd0000000-0000-0000-0000-000000000001';

delete from sales_layouts
 where tenant_id = 'd0000000-0000-0000-0000-000000000001';
