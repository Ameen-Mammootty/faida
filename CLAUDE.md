...
Its only source until WP-70 is the legacy shared token resolving to the seeded tenant as actor `console`; the worker path takes its tenant from the job payload (WP-72) and never from a default. RLS stays deny-all: tenancy is enforced in the application layer by decision (2026-09-03 Decision Log), and policy-based RLS is owed at the first second door to the database (`TODOS.md`).
