...
The shell around all six screens is `AppShell.tsx` (WP-97; `/incentive` joined it in M13): from 1280 px a Date Palm sidebar in three groups - 1280 and not 1024, so the content column stays about 1000 px, wider than the 976 px every fixed grid was measured at - and under that width the shipped top bar with the brand mark as the dashboard link, one chrome displayed at a time and one session read per page load.
...
The daily brief (M10): one message a morning to a phone the owner asked us to send it to, and every figure on it is the dashboard's own - `brief.py` composes it from two calls of `dashboard.read_dashboard` (the month to date for the figures, the branches and the items; the default 28 days for the price rises) and from nothing else, taking each payload's sentences byte for byte where it has them and composing one fixed shape in Python where it has only numbers, so nothing is generated and one figure can never be quoted in two sets of words (C15).
The five text lines - the day, the latest day's sales, the month so far, what the materials cost, and that cost's share of the sales it covers - and the picture card are built from the same `Kpi` pieces, so a tile and the line under it are one number by construction; the card is drawn by `brief_card.py` as a pure layout over that same `Brief` with a thin Pillow pass on top and the brand's fonts bundled (1080 by 1350, the tallest ratio WhatsApp shows whole in the bubble), byte-identical for the same brief, and stored immutably at `{tenant_id}/briefs/{brief_date}/{recipient_id}.png` before anything leaves the building.
The send is `wa.send_template` with that picture as the header and one outbound `wa_messages` row naming the template, the five values, the card and the morning's key, on which Meta's receipts are stamped by the webhook's `statuses` - `sent`, `delivered`, `read`, `failed` terminal, a stamp only ever moving forward, and a receipt that beats the record write leaving a stub the record upserts into, so neither order loses a fact.
`brief_recipients` (0022) is the one place a recipient lives and holds state alone - the phone, its IANA timezone, its local hour, `paused_at` - because who asked for a daily message about their own money, when, and on what evidence is the `brief_recipient.added` audit row written in the same transaction (C8 extended, P14).
The whole scheduler is the worker's own tick: once a minute on every pass of the loop, before a job is claimed, so a morning of extraction cannot starve it - it computes each recipient's local date with `zoneinfo`, logs and skips a timezone name that does not resolve, and enqueues one `send_brief` job per recipient per local day through `jobs_send_brief_uidx`, due from `send_at_local` (07:00) and skipped with one log line past `BRIEF_SEND_UNTIL_LOCAL` (noon) rather than sent late; `claim_job` now also reclaims a `running` row untouched for `STALE_RUNNING_MINUTES` for every job kind, so a hard kill costs ten minutes and not a morning (P12).
A tenant with nothing loaded sends nothing and says so in one log line; a recipient phone that is no branch's is stamped `ignored_brief_recipient` before anything else and answered once a day with `REPLY_BRIEF_RECIPIENT`, while a phone that is both is a branch (C2 as amended); `BRIEF_ENABLED` is the kill switch beside `WORKER_ENABLED`, and `paused_at` is the per-recipient one.
The words are the screen's: "materials share of costed sales" with the covered share beside it and never food cost %, contribution never called profit, nothing called verified, a figure the read withholds said in words with the read's own note rather than as a zero, and the first line naming the newest loaded day and its age, so "yesterday" is printed only on a morning when it is true.
The pilot's template is `faida_daily_brief`, approved 2026-09-11 as Marketing rather than Utility and accepted for the pilot because the test number is free either way; production gets the same five slots worded as the owner's own account statement (`faida_account_statement`, P13 as amended, plan.md §7.3 row 107).
`brief_cli` is the printed brief and the rehearsal send, running through the job's own `read_brief` and `send_brief_card` so a rehearsal proves the real path; `Docs/DEMO_RUNBOOK.md` §J is the operating manual.

The staff incentive (M13, built 2026-09-12 and 2026-09-13 off the spec in GitHub issue #6, tickets #7 to #15): the owner turns the dashboard's answer into a scheme month - the calendar month, frozen when created, with a net sales target, a percentage of net sales above it and an optional cap per branch, and push weeks Monday to Sunday clipped to the month, each carrying one chain-wide push list of menu items grouped by the menu's own category with a rate per portion and a portion target per branch - and the branch team earns a pool split by the three role shares (manager, supervisor, sales team; set per tenant, editable at any time, snapshotted onto the month at creation so a running month keeps the shares it started with).
The bonus is scored on what the till prints - portions net as the till prints them (D9, the opposite of usage's P9 for a stated reason) and net sales - and never on contribution, which appears once, fils-precise beside the rate box as the owner's guide (`Docs/adr/0001`); the vocabulary is `CONTEXT.md`, and the staff-facing word is bonus, never profit, profit share, commission or verified (a test calls the module and pins their absence).
`incentive.py` is pure and composes every figure and every sentence - the pool by D8, the split rounded to fils with the remainder to the sales share, "provisional, 26 of 30 days loaded" until every day of the month is loaded for that branch and "final" once approved, every refusal worded once so the API and the screen quote it - and the statement is read on every request out of the branch's own loaded sales days and stored nowhere; the only rows (migration 0023, the 0019 shape, deny-all RLS) are what the owner typed and what the owner approved.
Every write is one door with one audit row naming the actor: `PUT /api/incentive/shares` (`incentive.role_shares_set`), `POST /api/incentive/months` (every branch's targets in the one request, the weeks laid out, the shares snapshotted, `incentive.scheme_month_created`; last month's net sales beside each box as advice and never the baseline, D3), `PUT /api/incentive/weeks/{id}` (the week's list replaced in one transaction, `incentive.push_list_set`; a week under way or over is refused against the chain's own local date, and a dish with no till name, one archived, a blank rate or a branch without a target are refused by name, D5 and D7), and `POST /api/incentive/months/{id}/branches/{id}/approve`, the only door to a final statement: its own route in the cash approval's shape (actor, reason required, one `incentive.statement_approved` row), refusing a provisional month with the count, a blank reason and a second approval, and writing the approval row with the figures as a snapshot, its audit row and the final card's `send_scoreboard` job inside one transaction (`db.approve_statement`), so the three commit together or not at all.
A sales day inside an approved month stays replaceable through the sales-day door (D12): the statement keeps the approved figures, says "final; the till now says otherwise", and the screen prints what the till reads now under each figure that moved.
The scoreboard is one card a morning to the branch's registered phone (`branches.wa_phone_e164`, C2's one resolver) at `SCOREBOARD_SEND_AT_LOCAL` (07:00) in `branches.timezone`, composed by `scoreboard.py` from the statement and nothing else - the freshness line in the brief's own idiom, this week's push list with each item's portions against target and what a portion above target earns (`incentive.rate_words`, story 45, on the daily card alone), the month's net sales against target, the pool marked "so far", a hole named with no bar under it and never a nought, an empty week saying "no push list", ten rows drawn and the rest counted - drawn by `scoreboard_card.py` on the sheet it shares with the brief (`card.py`), byte-identical for the same input, and stored immutably at `{tenant_id}/scoreboards/{day}/{branch_id}.png` before the upload and the send; the final variant is the same composer and sheet (the month named as closed, every week's items up to eighteen rows, the split by role as "Manager share, 50%" and the two others to the fil, the till note when a day moved after approval), stored under the `-final` suffix.
The send is `worker.send_scoreboard_card` through the brief's own `send_card` door: one Meta template with the picture as header and two text slots (the headline and the freshness line, at most 160 characters each, D15; the name is the one constant `scoreboard.TEMPLATE_NAME`, the placeholder `faida_scoreboard` until the founder's submission, issue #16, is approved), and one outbound `wa_messages` row keyed on branch, day and variant, on which Meta's receipts land as the brief's do.
The scheduler is `worker.tick_scoreboards` on the worker's own tick beside the brief's - three small reads a minute whatever the number of chains - which enqueues one `send_scoreboard` job per branch per local day through `jobs_send_scoreboard_uidx` (branch, variant, day), never past the brief's `BRIEF_SEND_UNTIL_LOCAL` (noon), and gives nothing and one log line to a branch with no scheme month covering its local date, a paused branch, one with no phone or an unusable timezone, and a branch whose month is already final (`db.list_final_branch_months`; the job's own skip on a closed month is the second guard); the job skips a pause it meets on the way and reads the statement through `worker.read_scoreboard`, the one read it shares with `python -m faida_api.scoreboard_cli --tenant --branch [--day] [--final --month YYYY-MM] [--card out.png] [--send --to <phone>]`, the printed card and the rehearsal through the job's own door, recorded with `rehearsal: true` outside the day's key.
`INCENTIVE_ENABLED` is the kill switch beside `BRIEF_ENABLED`, and `POST /api/incentive/branches/{id}/pause` and `/resume` are the per-branch one (`incentive_branches.paused_at`, `incentive.branch_paused` and `.resumed`, no second row for a second click), which stops the daily card and never the final one.
The screen is `/incentive`, sixth in the shell, read whole by `GET /api/incentive?month=` in a fixed number of queries whatever the chain's size: the shares, the month picker, each branch's targets, the weeks as tabs with their lists and the kept-per-plate figure beside each rate, and each branch's statement card below with the approval control and its reason box on a complete provisional month, "Approved 2 Sep 2026: <reason>" on a final one, and the pause control; `incentiveScreen.ts` holds every rendering decision, and the mock's fixtures are generated by the shipped Python module.
`Docs/DEMO_RUNBOOK.md` §K is the operating manual; a roster and per-person scores, a per-branch push list, a WhatsApp door for the till export, a second card language and a cross-sell KPI each wait in `TODOS.md` for the trigger named there.
